...
export const workProjectsQuery = groq`*[_type == "project" && type == "work"]{
    _id, title, subtitle, slug, summary, techlist, relevance, image {
    asset-> {
        ...,
        metadata
      }
...
export const openSourceProjectsQuery = groq`*[_type == "project" && type == "creation"]{
    _id, title, subtitle, summary, techlist, slug, url, relevance
  } | order(relevance asc)`

export type OpenSourceProjectsQuery = Readonly<
  Pick<
    IProject,
    '_id' | 'title' | 'subtitle' | 'slug' | 'summary' | 'techlist'  | 'relevance' | 'url'
  >
>[]
